...
until mysql -h "${HS_DB_HOST}" -P "${HS_DB_PORT}" -u "${HS_DB_USER}" -p"${HS_DB_PASSWORD}" "${HS_DB_NAME}" -e '\q' >/dev/null 2>&1; do
    sleep 1
done
echo "  Database: up (OK)"
